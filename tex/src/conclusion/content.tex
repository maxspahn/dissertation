...
That question usually arises when a robot
takes an \textit{unnatural} road to reach a goal. The problem
is directly related to the definition of natural, because it
referes to the human understanding of \textit{best} route.
In robotics, the best route is usually defined as the
optimal route, which is the shortest route in the
configuration space. This shortest route in configuration
space, however is hard to find, because of the high
dimensionality and the constraints defined by obstacles.
In fact, the problem is so hard that it is usually
approximated by sampling-based methods, such as RRT.
This methods solves the problem by sampling configurations
until a sequence of valid configurations leading to the goal
configuration is found \cite{Karaman2011}.
The problem with this approach is
that only the connection between two subsequent samples can
be guaranteed to be the shortest, not the entire sequence.
Moreover, this often leads to highly jerky motions. Although,
plenty of research has been conducted to improve the paths,
by for example smoothing the sequence \cite{siegwart2011introduction},
the problem is
inherent to planning in the configuration space. As it is
very unlikely that we humans do that, paths generated by
this approach seems unusual to us. Methods presented in this
thesis did not plan over a long horizon in the configuration
space, but relied either purely on local \ac{tg} or used
global path planning in the workspace. In general, this
seems to produce more natural looking paths. Specifically,
the technique of shaping the configuration manifold might be
very close to what humans do when planning a path
