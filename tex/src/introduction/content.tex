...
Computational costs are also often mentioned when
...
justified. In the context of \ac{tg}, the main drawback of 
learning-based approaches is their lack of interpretability,
low generalizability, and the need for large amounts of
training data.
...
metric tensor can depend on the position in the space.
...
resulting the geometry, a desired trajectory becomes
equivalent to the shortest path in this geometry, making
computations efficient and the resulting trajectory
intuitive.

In this dissertation, we recall and extend a framework that
allows to design \ac{tg} by iteratively shaping the geometry
of the configuration space. Specifically, individual
