...
\MS{Rewrite with ChatGPT.}
%
In this section, we very briefly introduce the concepts required for trajectory
generation with \ac{fabrics}. For a more in-depth introduction to \ac{fabrics}
and its foundations in differential geometry, the reader
is referred to \cite{Ratliff2021,Spahn2023a,Wyk2022}.
%
...
%
Inspired by simple mechanics (e.g., the simple pendulum), the framework of
\ac{fabrics} designs motion policies as second-order dynamical systems $\xddot =
\pi(\x,\xdot)$~\cite{Cheng2020,Ratliff2020}. The motion policy is defined by the
...
them as \textit{spectral semi-sprays}, or \textit{specs} for short.  When the
space of the task variable is clear from the context, we drop the subscript. 
...
avoidance, etc. The power of \ac{fabrics} lies in the metric-weighted sum to
...
behavior of the system. \Ac{fabrics} introduce construction rules to
...
of two \ac{fabrics} is, itself, an optimization fabric.
