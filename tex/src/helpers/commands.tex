...
\newcommand\fkarg[1]{\ensuremath{\textrm{fk}_{\textrm{#1}}}}
...
\newcommand{\rr}{\ensuremath{r_{\textrm{resolution}}}}
...
\acrodef{mppi}[MPPI]{Model Predictive Path Integral Control}
\acrodef{fsd}[FSD]{Free Space Decomposition}
\acrodef{apf}[APF]{Artificial Potential Fields}
\acrodef{dmp}[DMPs]{Dynamic Movement Primitives}
\acrodef{osc}[OSCs]{Operational Space Control}
\acrodef{rmp}[RMPs]{Riemannian Motion Policies}
\acrodef{ic}[IC]{Impedance Control}
\acrodef{cic}[CIC]{Cartesian Impedance Control}
...
\acrodef{sdf}[SDF]{Signed Distance Fields}
...
\newcommand{\ralsampling}{
  \publication
  {C. Pezzato$^*$, C. Salmi$^*$, E. Trevisan$^*$, \textbf{M. Spahn}, J.
  Alonso-Mora and C.H. Corbato}
  {Sampling-based model predictive control leveraging parallelizable physics simulations}
  {\textit{Submitted to} \acl{ral}}
  {2024}
}
